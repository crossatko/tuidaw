...
| Recording | `tuidaw_start_recording(id)`, `tuidaw_stop_recording(id)`, `tuidaw_get_recording_buffer/length`, `tuidaw_get_recording_start_playhead(id)`                                                                             |
...
- **Recording**: per-track capture devices with linear buffers (10 min max), polled from JS via `pollRecordingData()`. Native `rec_start_playhead` captures playhead on first sample arrival for latency compensation. JS uses pre-allocated preview buffers with doubling growth (no O(n) realloc per frame). `updateTrackSamples()` skipped during recording (native skips playback for recording tracks). Multi-channel capture: opens device in native channel count when specific channel selected, extracts in callback. Web uses getUserMedia + ScriptProcessorNode (WASM capture unreliable).
...
- **JACK client must stay alive between monitoring toggles**: Closing and reopening the JACK client on every monitoring off→on cycle causes PipeWire graph reconfiguration, which produces crackling/distortion (~30-40% of toggles, first enable always affected). Fix: `tuidaw_stop_monitoring()` only sets `monitoring=0` — the JACK process callback already outputs silence when the flag is off. `tuidaw_start_monitoring()` detects the existing slot (`jack_mon_active && jack_mon_slot >= 0`) and just sets `monitoring=1` — instant re-enable with zero graph reconfiguration.
- **Shared JACK client for all tracks**: Per-track JACK clients cause PipeWire graph reconfiguration when enabling monitoring on additional tracks. Fix: single `tuidaw-monitor` client in `EngineState` shared by all tracks. Each track claims a pre-registered port slot (index 0-7) and connects/disconnects. Process callback iterates all tracks, routing audio by slot index.
- **JACK ports must be registered BEFORE activate()**: Calling `jack_port_register()` on an already-active JACK client triggers PipeWire graph reconfiguration per port. Fix: register all input ports before `jack_activate()` so PipeWire sees the complete port set in a single graph update.
- **JACK capture-only + ring buffer playback**: JACK output ports competing with miniaudio on the same ALSA sink forces PipeWire to insert a mixer node, causing graph disruption that corrupts the custom ALSA node's Implicit Feedback Mode streaming. Fix: JACK client is capture-only (input ports only, zero output ports). Captured audio goes into a per-track lock-free ring buffer (`mon_ring[8192]`, ~170ms at 48kHz). The miniaudio playback callback reads from the ring buffer and mixes monitoring audio into stereo output with volume/pan. This eliminates all JACK→Scarlett connections. Recording also uses the same JACK capture data. Both monitoring and recording are affected by capture-level distortion (same source).
...
- **Recording latency compensation**: Without compensation, recorded audio is systematically placed late on the timeline by the capture device startup latency (~20-50ms for PulseAudio, ~0-21ms for JACK piggyback). Fix: `rec_start_playhead` field in `TrackState` captures the native playhead atomically when the first audio sample arrives in the capture callback. JS uses this instead of the pre-transport JS-side estimate. All 3 recording paths (PulseAudio capture, JACK monitor piggyback, JACK record-only) set `rec_start_playhead` on first `write_pos == 0` callback.
- **Recording buffer realloc storm**: Merging recorded audio into `track.samples` every 33ms poll with `new Float32Array(totalLen)` + full copy causes severe GC pressure (10+ GB allocations over 60s). Fix: pre-allocated preview buffer with doubling growth, append-only writes, `updateTrackSamples()` skipped entirely during recording (native engine skips playback for armed tracks anyway — `if (recording) continue` in `playback_callback`). Final merge done once on stop.
...
TUI: native capture devices with linear buffers, polled every ~33ms. Native `rec_start_playhead` provides sample-accurate timeline placement. JS preview uses pre-allocated buffers with doubling growth; no `updateTrackSamples()` during recording (native skips playback for armed tracks). Final merge on stop. Web: getUserMedia + ScriptProcessorNode per armed track, shared DeviceCapture per device (ref-counted). Punch-in/out during transport.
