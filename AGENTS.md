...
- **Click distortion at non-1.0x speeds**: Click was using content-space playhead for beat timing, but WSOLA advances the playhead at `speed * hop` rate through content-space. Fix: dedicated `click_pos` wall-clock counter that advances at 1 frame per output frame regardless of WSOLA speed. Reset on play/seek only. On BPM change, snap to nearest beat boundary. Never reset on enable/disable toggle — resetting on toggle causes the first click after enabling to fire immediately (offbeat).
- **Click loop realignment**: On loop wrap, `click_pos` is decremented by `loop_len` (not realigned with a phase formula). This keeps the wall-clock beat phase perfectly continuous — `click_pos % samples_per_beat` is unchanged across the wrap, so no drift accumulates over multiple loops.
