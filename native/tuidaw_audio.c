...
static void playback_callback(ma_device* pDevice, void* pOutput, const void* pInput, ma_uint32 frameCount) {
    ...
    for (ma_uint32 frame = 0; frame < frameCount; frame++) {
        ...
        // Click (metronome) — pre-rendered long buffer approach.
        // The click_samples buffer contains the full click track: click tones
        // placed at exact beat positions with silence between them. The buffer
        // is generated by tuidaw_generate_click() using GCD-exact integer math.
        // We simply read click_samples[counter] with a bounds check.
        // No modulo, no floating-point BPM math, no fmod, no precision issues.
